...
async def DummyHandler(kazu):
    # clean chat actions
    key = udB.get_key("CLEANCHAT") or []
    if kazu.chat_id in key:
        try:
            await kazu.delete()
    ...
    # thank members
    if must_thank(kazu.chat_id):
        chat_count = (await kazu.client.get_participants(kazu.chat_id, limit=0)).total
        if chat_count % 100 == 0:
            stik_id = chat_count / 100 - 1
            sticker = stickers[stik_id]
            await kazu.respond(file=sticker)
    # force subscribe
    if (
        udB.get_key("FORCESUB")
        and ((kazu.user_joined or kazu.user_added))
        and get_forcesetting(kazu.chat_id)
    ):
        user = await kazu.get_user()
        if not user.bot:
            joinchat = get_forcesetting(kazu.chat_id)
            try:
                await kazu_bot(GetParticipantRequest(int(joinchat), user.id))
            except UserNotParticipantError:
                await kazu_bot.edit_permissions(
                    kazu.chat_id, user.id, send_messages=False
                )
                res = await kazu_bot.inline_query(
                    asst.me.username, f"fsub {user.id}_{joinchat}"
                )
                await res[0].click(kazu.chat_id, reply_to=kazu.action_message.id)

    if kazu.user_joined or kazu.added_by:
        user = await kazu.get_user()
        ...
        if udB.get_key("kazu_BANS"):
            try:
                ...
                if is_banned["is_banned"]:
                    await kazu.client.edit_permissions(
                        chat.id,
                        user.id,
                        view_messages=False,
                    )
                    await kazu.client.send_message(
                        chat.id,
                        f'**@kazuBans:** Banned user detected and banned!\n`{str(is_banned)}`.\nBan reason: {is_banned["reason"]}',
                    )
        ...
        if reason and chat.admin_rights:
            try:
                ...
                await kazu.reply(gban_watch)
            except Exception as er:
                LOGS.exception(er)

        # greetings
        elif get_welcome(kazu.chat_id):
            user = await kazu.get_user()
            chat = await kazu.get_chat()
            title = chat.title or "this chat"
            count = (
                chat.participants_count
                or (await kazu.client.get_participants(chat, limit=0)).total
            )
            ...
            if wel.get("button"):
                ...
                await something(kazu, msg, med, btn)
            elif msg:
                send = await kazu.reply(
                    msg,
                    file=med,
                )
                ...
                await kazu.reply(file=med)
    elif (kazu.user_left or kazu.user_kicked) and get_goodbye(kazu.chat_id):
        user = await kazu.get_user()
        chat = await kazu.get_chat()
        title = chat.title or "this chat"
        count = (
