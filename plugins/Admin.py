...
async def prmte(kazu):
    xx = await kazu.eor(get_string("com_1"))
    user, rank = await get_uinfo(kazu)
    rank = rank or "Admin"
    FullRight = False
    if not user:
    ...
    try:
        if FullRight:
            await kazu.client(
                EditAdminRequest(kazu.chat_id, user.id, kazu.chat.admin_rights, rank)
            )
        else:
            await kazu.client.edit_admin(
                kazu.chat_id,
                user.id,
                invite_users=True,
                ban_users=True,
        ...
        await eod(
            xx, get_string("pro_2").format(inline_mention(user), kazu.chat.title, rank)
        )
...
async def dmote(kazu):
    ...
    try:
        ...
        await eod(xx, get_string("de_2").format(inline_mention(user), kazu.chat.title))
    except Exception as ex:
        return await xx.edit(f"`{ex}`")
...
async def bban(kazu):
    something = await get_uinfo(kazu)
    if not something:
        return
    user, reason = something
    if not user:
        return await eod(kazu, get_string("ban_1"))
    if user.id in DEVLIST:
        return await eod(kazu, get_string("ban_2"))
    try:
        await kazu.client.edit_permissions(kazu.chat_id, user.id, view_messages=False)
    except UserIdInvalidError:
        return await eod(kazu, get_string("adm_1"))
    except BadRequestError:
        return await eod(kazu, get_string("ban_3"))
    senderme = inline_mention(await kazu.get_sender())
    userme = inline_mention(user)
    text = get_string("ban_4").format(userme, senderme, kazu.chat.title)
    if reason:
...
async def uunban(kazu):
    xx = await kazu.eor(get_string("com_1"))
    if kazu.text[1:].startswith("unbanall"):
        return
    something = await get_uinfo(kazu)
    if not something:
        return
    ...
    try:
        await kazu.client.edit_permissions(kazu.chat_id, user.id, view_messages=True)
    except UserIdInvalidError:
        return await eod(kazu, get_string("adm_1"))
    except BadRequestError:
        return await xx.edit(get_string("adm_2"))
    sender = inline_mention(await kazu.get_sender())
    text = get_string("unban_3").format(inline_mention(user), sender, kazu.chat.title)
    if reason:
        text += get_string("ban_5").format(reason)
    await xx.edit(text)
...
async def kck(kazu):
    if "kickme" in kazu.text:
        return
    if kazu.is_private:
        return await kazu.eor("`Gunakan ini di Grup/Saluran.`", time=5)
    ml = kazu.text.split(" ", maxsplit=1)[0]
    xx = await kazu.eor(get_string("com_1"))
    something = await get_uinfo(kazu)
    if not something:
        return
    ...
    try:
        await kazu.client.kick_participant(kazu.chat_id, user.id)
    ...
    text = get_string("kick_4").format(
        inline_mention(user), inline_mention(await kazu.get_sender()), kazu.chat.title
    )
    if reason:
        text += get_string("ban_5").format(reason)
    await xx.edit(text)
...
async def unp(kazu):
    xx = await kazu.eor(get_string("com_1"))
    ch = (kazu.pattern_match.group(1).strip()).strip()
    msg = None
    if kazu.is_reply:
        msg = kazu.reply_to_msg_id
    elif ch != "all":
        return await xx.edit(get_string("unpin_1").format(HNDLR))
    try:
        await kazu.client.unpin_message(kazu.chat_id, msg)
    except BadRequestError:
        return await xx.edit(get_string("adm_2"))
    except Exception as e:
...
async def pin_message(kazu):
    match = kazu.pattern_match.group(1).strip()
    if not kazu.is_reply:
        return await kazu.eor("`Balas pesan..`", time=6)
    if not match:
        return await kazu.eor("`Harap sediakan waktu..`", time=8)
    msg = await kazu.eor(get_string("com_1"))
    msg_id = kazu.reply_to_msg_id
    try:
        time = ban_time(match)
        await kazu.client.pin_message(kazu.chat_id, msg_id)
        await msg.eor(f"`disematkan waktu` `{time}`", time=8)
    except Exception as er:
        return await msg.edit(str(er))
    await asyncio.sleep(time)
    try:
        await kazu.client.unpin_message(kazu.chat_id, msg_id)
    except Exception as er:
        LOGS.exception(er)
...
async def autodelte(kazu):
    match = kazu.pattern_match.group(1).strip()
    if not match or match not in ["24h", "7d", "1m", "off"]:
        return await kazu.eor("`Silakan Gunakan dalam Format yang Tepat..`", time=5)
    if match == "24h":
        tt = 3600 * 24
    elif match == "7d":
        tt = 3600 * 24 * 7
        ...
        tt = 0
    try:
        await kazu.client(SetHistoryTTLRequest(kazu.chat_id, period=tt))
    except ChatNotModifiedError:
        return await kazu.eor(
            f"Pengaturan Hapus Otomatis Sudah sama dengan `{match}`", time=5
        )
    await kazu.eor(f"Status Hapus Otomatis Diubah menjadi `{match}` !")
