...
async def play_music_(event):
    ...
    kazuSongs = Player(chat, event)
    song_name = f"{song_name[:30]}..."
    if not kazuSongs.group_call.is_connected:
        if not (await kazuSongs.vc_joiner()):
            return
        await kazuSongs.group_call.start_audio(song)
...
async def mute(event):
    if len(event.text.split()) > 1:
        ...
        chat = event.chat_id
    kazuSongs = Player(chat)
    await kazuSongs.group_call.set_is_mute(True)
...
async def unmute(event):
    if len(event.text.split()) > 1:
        ...
        chat = event.chat_id
    kazuSongs = Player(chat)
    await kazuSongs.group_call.set_is_mute(False)
...
async def pauser(event):
    if len(event.text.split()) > 1:
        ...
        chat = event.chat_id
    kazuSongs = Player(chat)
    await kazuSongs.group_call.set_pause(True)
...
async def resumer(event):
    if len(event.text.split()) > 1:
        ...
        chat = event.chat_id
    kazuSongs = Player(chat)
    await kazuSongs.group_call.set_pause(False)
...
async def replayer(event):
    if len(event.text.split()) > 1:
        ...
        chat = event.chat_id
    kazuSongs = Player(chat)
    kazuSongs.group_call.restart_playout()
...
async def live_stream(e):
    ...
    kazuSongs = Player(chat, e)
    if not kazuSongs.group_call.is_connected and not (await kazuSongs.vc_joiner()):
        return
    ...
    await xx.delete()
    await kazuSongs.group_call.start_audio(file)
...
async def rejoiner(event):
    ...
    kazuSongs = Player(chat)
    try:
        await kazuSongs.group_call.reconnect()
...
async def skipper(event):
    if len(event.text.split()) > 1:
        ...
        chat = event.chat_id
    kazuSongs = Player(chat, event)
    await kazuSongs.play_from_queue()
...
async def video_c(event):
    ...
    await kazuSongs.group_call.start_video(song, with_audio=True)
    await xx.delete()
