...
async def lol(kazu):
    ...
    if match in ["inline", "i"]:
        try:
            res = await kazu.client.inline_query(asst.me.username, "alive")
            return await res[0].click(kazu.chat_id)
    ...
    if pic:
        try:
            ...
            return await kazu.try_delete()
        except ChatSendMediaForbiddenError:
            pass
        except BaseException as er:
            LOGS.exception(er)
            try:
                await kazu.reply(file=pic)
                await kazu.reply(
                    als,
                    parse_mode=parse,
                    buttons=buttons if inline else None,
...
async def restart(kazu):
    ok = await kazu.eor(get_string("bot_5"))
    call_back()
    who = "bot" if kazu.client._bot else "user"
    udB.set_key("_RESTART", f"{who}_{kazu.chat_id}_{ok.id}")
    if heroku_api:
...
async def inline_alive(kazu):
    ...
    if pic:
        try:
            ...
            return await kazu.answer(results)
        except BaseException as er:
            LOGS.info(er)
    result = [
...
async def _(e):
    ...
    if m:
        ...
        await xx.edit(
            f'<code>Your BOT is </code><strong>up-to-date</strong><code> with </code><strong><a href="https://github.com/senpai80/kazu/tree/{branch}">[{branch}]</a></strong>',
            parse_mode="html",
            link_preview=False,
        )
