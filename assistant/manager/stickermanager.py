...
from telethon.utils import get_displkazu_name, get_input_document
...
async def kang_cmd(kazu):
    sender = await kazu.get_sender()
    if not isinstance(sender, User):
        return
    if not kazu.is_reply:
        return await kazu.eor("`Reply to a sticker/photo..`", time=5)
    ...
    try:
        emoji = kazu.text.split(maxsplit=1)[1]
    except IndexError:
        emoji = None
    if reply.sticker:
        ...
        return await kazu.eor("`Reply to sticker or text to add it in your pack...`")
    if not emoji:
        emoji = "🏵"
    if dl:
        upl = await kazu.client.upload_file(dl)
        file = get_input_document(
            await kazu.client(UploadMediaRequest(InputPeerSelf(), upl))
        )
    get_ = udB.get_key("STICKERS") or {}
    type_ = "anim" if animated else "static"
    if not get_.get(kazu.sender_id) or not get_.get(kazu.sender_id, {}).get(type_):
        sn = f"{pre}_{kazu.sender_id}"
        title = f"{get_displkazu_name(sender)}'s Kang Pack"
        if animated:
            type_ = "anim"
            sn += "_anim"
            title += " (Animated)"
        ...
        try:
            ...
            sn = sn.replace(str(kazu.sender_id), f"{kazu.sender_id}_{kazu.id}")
        except StickersetInvalidError:
            pass
        try:
            pack = await kazu.client(
            ...
            return await kazu.eor(str(er))
        sn = pack.set.short_name
        if not get_.get(kazu.sender_id):
            get_.update({kazu.sender_id: {type_: [sn]}})
        else:
            get_[kazu.sender_id].update({type_: [sn]})
        udB.set_key("STICKERS", get_)
        return await kazu.reply(
            f"**Kanged Successfully!\nEmoji :** {emoji}\n**Link :** [Click Here](https://t.me/addstickers/{sn})"
        )
    name = get_[kazu.sender_id][type_][-1]
    try:
        await asst(GetSticker(InputStickerSetShortName(name), hash=0))
    except StickersetInvalidError:
        get_[kazu.sender_id][type_].remove(name)
    try:
        await asst(
        ...
        sn = f"{pre}{kazu.sender_id}_{kazu.id}"
        title = f"{get_displkazu_name(sender)}'s Kang Pack"
        if animated:
            sn += "_anim"
            title += " (Animated)"
        ...
        try:
            ...
            return await kazu.eor(str(er))
        get_[kazu.sender_id][type_].append(pack.set.short_name)
        udB.set_key("STICKERS", get_)
        return await kazu.reply(
        ...
        LOGS.exception(er)
        return await kazu.reply(str(er))
    await kazu.reply(
...
async def do_magic(kazu):
    ko = udB.get_key("STICKERS") or {}
    if not ko.get(kazu.sender_id):
        return await kazu.reply("No Sticker Pack Found!")
    al_ = []
    ul = ko[kazu.sender_id]
    for _ in ul.keys():
        al_.extend(ul[_])
    msg = "• **Stickers Owned by You!**\n\n"
    for _ in al_:
        try:
            pack = await kazu.client(GetSticker(InputStickerSetShortName(_), hash=0))
