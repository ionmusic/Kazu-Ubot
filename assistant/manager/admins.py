...
@callback(re.compile("cc_(.*)"), func=_kazu_cache.get("admin_callback"))
async def callback_(event):
    data = event.data_match.group(1).decode("utf-8")
    if data not in _kazu_cache.get("admin_callback", {}):
        return
    try:
    ...
    _kazu_cache["admin_callback"].update({data: (event.sender, perm)})
